				...
				InLine: &InLine{
					...
					ViewableImpression: &ViewableImpression{
						...
						Viewable: []CDATAString{
							{"http://viewable1.track.com"},
							{"http://viewable2.track.com"},
						},
						NotViewable: []CDATAString{
							{"http://notviewable1.track.com"},
							{"http://notviewable2.track.com"},
						},
						ViewUndetermined: []CDATAString{
							{"http://viewundetermined1.track.com"},
							{"http://viewundetermined2.track.com"},
						},
					},
					Creatives: []Creative{
						{
...
func TestQuickStart(t *testing.T) {
	...
	want := []byte(`{"Version":"4.2","Ad":[{"InLine":{"AdSystem":{"Name":"DSP"},"AdTitle":"adTitle","Impressions":[{"ID":"11111","URI":"http://impressionv1.track.com"},{"ID":"11112","URI":"http://impressionv2.track.com"}],"Extensions":[{"Type":"ClassName","Data":"AdsVideoView"},{"Type":"ExtURL","Data":"http://xxxxxxxx"}],"Creatives":[{"UniversalAdID":null,"Linear":{"Duration":"00:00:15","MediaFiles":[{"URI":"http://mp4.res.xxx.com/new_video/2020/01/14/1485/335928CBA9D02E95E63ED9F4D45DF6DF_20200114_1_1_1051.mp4","Delivery":"progressive","Type":"video/mp4","Width":1024,"Height":576}],"TrackingEvents":[{"Event":"start","URI":"http://track.xxx.com/q/start?xx"},{"Event":"firstQuartile","URI":"http://track.xxx.com/q/firstQuartile?xx"},{"Event":"midpoint","URI":"http://track.xxx.com/q/midpoint?xx"},{"Event":"thirdQuartile","URI":"http://track.xxx.com/q/thirdQuartile?xx"},{"Event":"complete","URI":"http://track.xxx.com/q/complete?xx"}],"SkipOffset":"00:00:05"},"ID":"987"}]},"ID":"123"}],"Mute":true}`)
...
func TestEmptyVast(t *testing.T) {
	v := VAST{
		Version: "4.2",
		Errors: []CDATAString{
			{"http://xx.xx.com/e/error?e=__ERRORCODE__&co=__CONTENTPLAYHEAD__&ca=__CACHEBUSTING__&a=__ASSETURI__&t=__TIMESTAMP__&o=__OTHER__"},
		},
	}
	want := []byte(`{"Version":"4.2","Errors":[{"Data":"http://xx.xx.com/e/error?e=__ERRORCODE__\u0026co=__CONTENTPLAYHEAD__\u0026ca=__CACHEBUSTING__\u0026a=__ASSETURI__\u0026t=__TIMESTAMP__\u0026o=__OTHER__"}]}`)
	got, err := json.Marshal(v)
	if err != nil {
		t.Errorf("Marshal():\n\terror = %v", err)
		return
	...
	want = []byte(`<VAST version="4.2"><Error><![CDATA[http://xx.xx.com/e/error?e=__ERRORCODE__&co=__CONTENTPLAYHEAD__&ca=__CACHEBUSTING__&a=__ASSETURI__&t=__TIMESTAMP__&o=__OTHER__]]></Error></VAST>`)
...
func BenchmarkVastMarshalJson(b *testing.B) {
	...
	want := []byte(`{"Version":"4.2","xmlns":"http://www.iab.com/VAST","Ad":[{"ID":"123","InLine":{"AdSystem":{"Data":"DSP"},"AdTitle":{"Data":"ad title"},"Impressions":[{"ID":"456","URI":"http://impression.track.cn"}],"Creatives":[{"ID":"123456","Linear":{"Duration":"00:00:15","TrackingEvents":[{"Event":"start","URI":"http://track.xxx.com/q/start?xx"}],"MediaFiles":[{"Delivery":"progressive","Type":"video/mp4","Width":1024,"Height":576,"URI":"http://mp4.res.xxx.com/new_video/2020/01/14/1485/335928CBA9D02E95E63ED9F4D45DF6DF_20200114_1_1_1051.mp4"}]}}]}}]}`)
...
func TestCreateVastJson(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "testCase1",
			want:    []byte(`{"Version":"4.2","xmlns":"http://www.iab.com/VAST","Ad":[{"InLine":{"AdSystem":{"Name":"DSP"},"AdTitle":"ad title","Impressions":[{"ID":"456","URI":"http://impression.track.cn"}],"Creatives":[{"UniversalAdID":null,"Linear":{"Duration":"00:00:15","MediaFiles":[{"URI":"http://mp4.res.xxx.com/new_video/2020/01/14/1485/335928CBA9D02E95E63ED9F4D45DF6DF_20200114_1_1_1051.mp4","Delivery":"progressive","Type":"video/mp4","Width":1024,"Height":576}],"TrackingEvents":[{"Event":"start","URI":"http://track.xxx.com/q/start?xx"}]},"ID":"123456"}]},"ID":"123"}]}`),
			wantErr: false,
		},
	}
...
func TestCreateVastXML(t *testing.T) {
	tests := []struct {
		...
		{
			name: "testCase1", want: []byte(`<VAST version="4.2" xmlns="http://www.iab.com/VAST"><Ad id="123"><InLine><AdSystem>DSP</AdSystem><AdTitle>ad title</AdTitle><Impression id="456"><![CDATA[http://impression.track.cn]]></Impression><Creatives><Creative id="123456"><Linear><Duration>00:00:15</Duration><MediaFiles><MediaFile delivery="progressive" type="video/mp4" width="1024" height="576"><![CDATA[http://mp4.res.xxx.com/new_video/2020/01/14/1485/335928CBA9D02E95E63ED9F4D45DF6DF_20200114_1_1_1051.mp4]]></MediaFile></MediaFiles><TrackingEvents><Tracking event="start"><![CDATA[http://track.xxx.com/q/start?xx]]></Tracking></TrackingEvents></Linear></Creative></Creatives></InLine></Ad></VAST>`),
			wantErr: false,
		},
	}
	for _, tt := range tests {
...
func loadFixture(path string) (*VAST, string, string, error) {
	...
	var vast VAST
	if err := xml.Unmarshal(raw, &vast); err != nil {
		return nil, "", "", err
	}

	x, err := xml.MarshalIndent(vast, "", "  ")
	if err != nil {
		return nil, "", "", err

	}

	return &vast, string(raw), string(x), err
}
...
func TestInlineLinear(t *testing.T) {
	vast, _, _, err := loadFixture("testdata/vast_inline_linear.xml")
	if !assert.NoError(t, err) {
		return
	}

	assert.Equal(t, "2.0", vast.Version)
	if assert.Len(t, vast.Ads, 1) {
		...
		if assert.NotNil(t, ad.InLine) {
			...
			if assert.Len(t, inline.Errors, 2) {
				assert.Equal(t, CDATAString{"http://myErrorURL/error"}, inline.Errors[0])
				assert.Equal(t, CDATAString{"http://myErrorURL/error2"}, inline.Errors[1])
			}
...
func TestWrapperNonLinear(t *testing.T) {
	v, raw, x, err := loadFixture("testdata/vast_wrapper_nonlinear_1.xml")
	if !assert.NoError(t, err) {
		return
	}
	fmt.Printf("RAW: %s\n", raw)
	...
	if assert.Len(t, v.Ads, 1) {
		...
		if assert.NotNil(t, ad.Wrapper) {
			...
			if assert.Len(t, wrapper.Impressions, 1) {
				assert.Equal(t, "http://myTrackingURL/wrapper/impression", string(wrapper.Impressions[0].URI))
			}
...
func TestSpotXVpaid(t *testing.T) {
	...
	if assert.Len(t, v.Ads, 1) {
		...
		if assert.NotNil(t, ad.InLine) {
			...
			assert.Equal(t, "", inline.Description)
			...
			if assert.Len(t, inline.Creatives, 2) {
				...
				if assert.NotNil(t, crea1.Linear) {
					linear := crea1.Linear
					b, err := os.ReadFile("testdata/spotx_adparameters.txt")
					if err != nil {
						assert.FailNow(t, "Cannot open adparams file")
					}
				...
				if assert.NotNil(t, crea2.CompanionAds) {
					if assert.Len(t, crea2.CompanionAds.Companions, 3) {
						...
						if assert.NotNil(t, companionAds2.HTMLResource) {
							b, err := os.ReadFile("testdata/spotx_html_resource.html")
							if err != nil {
			...
			if assert.Len(t, *exts, 2) {
				...
				assert.Equal(t, "<Price model=\"CPM\" currency=\"USD\" source=\"spotxchange\"><![CDATA[3.06]]></Price>", strings.TrimSpace(ext1.Data))
				ext2 := (*exts)[1]
				assert.Equal(t, "SpotX-Count", ext2.Type)
				assert.Equal(t, "<total_available><![CDATA[1]]></total_available>", strings.TrimSpace(ext2.Data))
			}
		}
	}
}
